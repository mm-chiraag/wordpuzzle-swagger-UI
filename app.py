        ...
        tasks.append({
            ...
            "action_schema": {
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "description": f"{cfg['word_length']}-letter word guess"
                    }
                },
                "required": ["action"],
            },
...
async def step(request: Request):
    ...
    try:
        ...
        return JSONResponse(status_code=200, content={
            "error": str(e), "done": True, "reward": 0.0
        })
...
async def grader(request: Request):
    ...
    if target:
        env._target_word = target.lower().strip()

    if not guess or len(guess) != env.word_length:
        return JSONResponse(status_code=200, content={
            "score":   0.0,
            "valid":   False,
